...
Note the sign: the parallel axis theorem for products of inertia *adds* $m\,x_C\,y_C$, but the inertia tensor has negative signs on off-diagonal terms ($-I_{xy}$), so when populating the tensor the net effect depends on your sign convention. In the convention used in this course (negative off-diagonals in the tensor), the formula is:

$${}^{C}I_{xy} = {}^{A}I_{xy} - m\,x_C\,y_C$$

This negative sign arises because the off-diagonal entries of the inertia tensor carry a negative sign by convention.
